<template>
  <div>
    <Head title="Parrafos"/>
    <h1 class="mb-8 text-3xl font-bold">Parrafos</h1>
    ...
    <div class="bg-white rounded-md shadow overflow-x-auto">
      <table class="w-full whitespace-nowrap">
        <tr class="text-left font-bold">
          ...
          <th class="pb-4 pt-6 px-6" colspan="2">Parrafo</th>
        </tr>
        <tr v-for="parrafo in parrafos.data" :key="parrafo.id" class="hover:bg-gray-100 focus-within:bg-gray-100">
          <td class="border-t">
          ...
          <td class="border-t">
            <Link class="flex items-center px-6 py-4 focus:text-indigo-500" :href="`/parrafos/${parrafo.id}/edit`">
              {{ parrafo.doc_descripcion }}
              <icon v-if="parrafo.deleted_at" name="trash" class="flex-shrink-0 ml-2 w-3 h-3 fill-gray-400"/>
            </Link>
          </td>
          <td class="border-t">
            <Link class="flex items-center px-6 py-4" :href="`/parrafos/${parrafo.id}/edit`" tabindex="-1">
              <div v-if="parrafo.cap_descripcion">
                {{ parrafo.cap_descripcion }}
              </div>
            </Link>
          </td>
          <td class="border-t">
            <Link class="flex items-center px-6 py-4" :href="`/parrafos/${parrafo.id}/edit`" tabindex="-1">
              <div v-if="parrafo.par_descripcion">
                {{ parrafo.par_descripcion }}
              </div>
            </Link>
          </td>
  ...
  watch: {
    form: {
      ...
      handler: throttle(function () {
        this.$inertia.get('/parrafos', pickBy(this.form), {preserveState: true})
      }, 150),
    },
  },
  methods: {
